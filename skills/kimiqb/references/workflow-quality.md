...
- Long Kimi Code session handoffs should state outcome, unchanged boundaries, validation checkpoints, stop gates, token/context risk, and whether subagents are recommended.
...
## Keep Long-Running Kimi Code Session Output Concise
...
- Step 2 must read optional `Project-Ontology.md` and `Planing-Ledger.md` when present, then finish by handing off Step 3 as text for new Kimi Code session.
